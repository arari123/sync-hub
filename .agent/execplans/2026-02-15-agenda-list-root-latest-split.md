...
| REQ-005 | 좌/우 패널 모두 썸네일이 제목 왼쪽에 표시되는지 확인 | 수동 확인 |
| REQ-006 | 답변 유무와 무관하게 카드 높이가 동일한지 확인 | 수동 확인 |
...
3. 좌/우 패널 모두 썸네일을 “제목 왼쪽”에 배치하고(기존 카드와 동일 크기), 답변 유무에 따른 카드 높이가 동일하도록 레이아웃을 조정한다.
4. 라벨/배경/구분선을 적용해 좌/우 패널이 명확히 구분되도록 디자인을 정리한다.
5. Docker 환경에서 `verify_fast` 및 프론트 빌드를 실행해 검증한다.
