...
| :--- | :--- | :--- |
| REQ-001~009 | 메인 프로젝트 카드 UI/링크/딥링크/상태 표기/검토 단계 타임라인/페이지네이션 확인 | `docker exec -w /app synchub_frontend npm run build` |
| REQ-001~009 | 회귀/문법/디자인 토큰/유닛 테스트 | `docker exec -w /app synchub_web bash scripts/verify_fast.sh` |
...
9. 프로젝트 리스트 하단 페이지네이션을 10개 단위 숫자 페이지 + `맨앞`/`맨뒤` 버튼 형태로 개선한다.
10. Docker에서 `verify_fast` + 프론트 `build`를 실행한다.
11. 변경을 커밋하고 `git push` 한다.
