...
## 1. Goal
`/home` 프로젝트 리스트에서 단계(설계/제작/설치/AS/종료) 기반 프로그레스바와 일정관리 연동 날짜 표시를 제공한다. 검토 단계는 오버레이로 생성일자와 함께 표시한다.
...
| :--- | :--- | :--- |
| FE-REQ-001~004 | `/home`에서 단계 바/날짜/검토 오버레이 확인 | 브라우저에서 확인 |
...
3. 단계별 바(5단계) + 날짜 그리드를 렌더링하고, 검토 단계는 오버레이를 표시한다.
4. Docker에서 `verify:fast` 및 프론트 `npm run build` 실행.
5. 커밋/푸시.
