...
- 2026-02-08 (예산 관리 드롭다운 2초 유지 UX)
  ...
  - 검증:
    - `docker-compose exec -T frontend sh -lc 'cd /app && npm run build'` 통과
    - `docker-compose exec -T web bash -lc 'cd /app && bash scripts/verify_fast.sh'` 통과 (`Ran 77 tests ... OK`)
- 2026-02-08 (브레드크럼/메뉴 일체화 + 드롭다운 유지 1초 조정)
  - 요구 반영:
    - 예산 관리 드롭다운 유지시간을 2초에서 1초로 단축
    - 브레드크럼과 프로젝트 컨텍스트 메뉴를 동일 헤더 블록으로 일체화
    - 창폭이 줄어도 브레드크럼이 줄바꿈되지 않고 한 줄 유지되도록 보정
  - 조치:
    - 드롭다운 닫힘 타이머 `2000ms -> 1000ms` 조정
    - `ProjectPageHeader` 상단을 통합 바(브레드크럼+메뉴) 구조로 재배치
    - `BudgetBreadcrumb`에 `whitespace-nowrap`/`shrink-0` 적용 및 커스텀 클래스 지원 추가
    - 상단 통합 바는 가로 overflow를 허용해 좁은 화면에서 줄바꿈 대신 스크롤로 대응
  - 관련 파일:
    - `frontend/src/components/ProjectContextNav.jsx`
    - `frontend/src/components/ProjectPageHeader.jsx`
