...
| :--- | :--- | :--- |
| REQ-001~006 | 메인 프로젝트 카드 UI/링크/딥링크 동작 확인 | `docker exec -w /app synchub_frontend npm run build` |
| REQ-001~006 | 회귀/문법/디자인 토큰/유닛 테스트 | `docker exec -w /app synchub_web bash scripts/verify_fast.sh` |
...
7. Docker에서 `verify_fast` + 프론트 `build`를 실행한다.
8. 변경을 커밋하고 `git push` 한다.
