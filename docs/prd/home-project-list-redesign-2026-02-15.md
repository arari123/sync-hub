...
- REQ-007: 파츠/AS 프로젝트는 `시작(start)` 단계일 때 상태 텍스트를 `진행 중`으로 표시한다.
- REQ-008: 프로젝트가 `검토(review)` 단계일 때 타임라인 프로그레스바의 첫 단계(설비: 설계, 파츠/AS: 시작)가 활성화(진행) 상태로 보이지 않도록 비활성화 상태로 표시한다.
- REQ-009: 프로젝트 리스트 하단 페이지네이션은 10개 단위의 숫자 페이지를 표시하고, `맨앞`/`맨뒤` 버튼을 제공한다.
...
- AC-008: 파츠/AS 프로젝트가 `시작(start)` 단계일 때 상태 텍스트가 `진행 중`으로 보인다.
- AC-009: `검토(review)` 단계 프로젝트의 타임라인 프로그레스바에서 첫 단계가 활성화 상태로 보이지 않는다.
- AC-010: 프로젝트가 2페이지 이상일 때, 하단 페이지네이션에 10개 단위 숫자 페이지 버튼과 `맨앞`/`맨뒤` 버튼이 표시된다.
