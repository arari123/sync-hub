...
- 삭제 대상 프로젝트의 하위 프로젝트(AS 포함)도 함께 삭제된다.
- 삭제 시 프로젝트에 연결된 데이터는 함께 정리된다.
  - 예산 버전/설비
  - 일정(`budget_projects.schedule_wbs_json`)
  - 안건(스레드/엔트리/첨부/댓글) 및 첨부 파일
  - 프로젝트 연결 문서(`documents`) 및 업로드 파일
- 프로젝트 커버 이미지 파일은 다른 프로젝트에서 참조하지 않을 때만 함께 삭제된다.
