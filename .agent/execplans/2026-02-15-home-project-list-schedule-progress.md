...
## 1. Goal
`/home` 프로젝트 리스트에서 단계(설계/제작/설치/워런티) 기반 프로그레스바와 일정관리 연동 날짜 표시를 제공한다. 검토/종료 단계는 오버레이로 생성일자/종료일자와 함께 표시하며, 오버레이는 크게/반투명(글래스) 스타일로 가독성을 강화한다.
...
2. 설치 종료일을 워런티 시작일로 사용하고, 종료일은 워런티 시작 + 1년으로 계산한다.
